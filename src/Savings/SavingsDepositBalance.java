...
/*The SavingsDepositBalance class is created as a JFrame by extending the JFrame class.
The class also implements the ActionListner and MouseListender interfaces
in order for each button to use the interfaces.*/

public class SavingsDepositBalance extends JFrame implements ActionListener, MouseListener {

    // Instance variables are stated.
    JLabel yourSavingsDeposit;
    JLabel depositSavings;
    JButton next;
    BackgroundImage panel;
    public static double totalSavingsBalance;
    Color originalColor;
    DecimalFormat format = new DecimalFormat("#.##");

    // The constructor is created where instance variables are assigned values.
    public SavingsDepositBalance() {

        // Instantiating totalSavingsBalance with 0.
        totalSavingsBalance = 0;

        // Instantiating the yourSavingsBalance variable as a new JLabel.
        yourSavingsDeposit = new JLabel();
        yourSavingsDeposit.setText("Your total balance in your savings account is:");
        yourSavingsDeposit.setFont(new Font("Engebrechtre", Font.BOLD, 23));
        yourSavingsDeposit.setBounds(45, 30, 400, 20);
        yourSavingsDeposit.setForeground(new Color(253, 252, 202));

        // Instantiating the depositSavings variable as a new JLabel. The text is set
        // to the return value of the savingsDepositBalance method from the
        // SavingsDeposit class.
        depositSavings = new JLabel("$" + format.format(Savings.SavingsDeposit.savingsDepositBalance()),
                SwingConstants.CENTER);
        depositSavings.setFont(new Font("Engebrechtre", Font.BOLD, 40));
        depositSavings.setBounds(170, 70, 151, 41);
        depositSavings.setForeground(new Color(253, 252, 202));
        ...
        this.add(yourSavingsDeposit);
        this.add(depositSavings, BorderLayout.CENTER);
        this.add(next);
        this.add(panel);
