...
/*The CheckingsWithdrawlBalance class is created as a JFrame by extending the JFrame class.
The class also implements the ActionListner and MouseListender interfaces
in order for each button to use the interfaces.*/

public class CheckingsWithdrawlBalance extends JFrame implements ActionListener, MouseListener {

    // Instance variables are stated.
    JLabel withdrawlBalance;
    JLabel withdrawlCheckings;
    JButton next;
    ...
    public CheckingsWithdrawlBalance() {

        // Instantiating the withdrawlBalance variable as a new JLabel.
        withdrawlBalance = new JLabel();
        withdrawlBalance.setText("Your total balance in your checkings account is:");
        withdrawlBalance.setFont(new Font("Engebrechtre", Font.BOLD, 23));
        withdrawlBalance.setBounds(45, 30, 400, 20);
        withdrawlBalance.setForeground(new Color(253, 252, 202));

        // Instantiating the withdrawlCheckings variable as a new JLabel. The text is
        // set to the return value of the completeWithdrawlBalance method from the
        // CheckingsWithdrawl class.
        withdrawlCheckings = new JLabel("$" + format.format(Checkings.CheckingsWithdrawl.completeWithdrawlBalance()),
                SwingConstants.CENTER);
        withdrawlCheckings.setFont(new Font("Engebrechtre", Font.BOLD, 40));
        withdrawlCheckings.setBounds(170, 70, 151, 41);
        withdrawlCheckings.setForeground(new Color(253, 252, 202));
        ...
        this.add(withdrawlBalance);
        this.add(withdrawlCheckings, BorderLayout.CENTER);
        this.add(next);
        this.add(panel);
